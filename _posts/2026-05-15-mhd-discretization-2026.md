---
layout: post
title: "Discretizing the Magnetic Induction Equation"
date: 2026-05-15
category: methods
tags: [MHD, Numerical Schemes, Finite Difference, Surface Flux Transport, Linux HPC]
excerpt: "From the vector induction equation to a working finite-difference scheme for radial field evolution on a spherical shell, including the CFL stability limits that constrain the timestep."
---
...
This guide walks through the mathematical derivation and numerical discretization required to isolate and solve for the radial component of the magnetic field ($B_r$) on a two-dimensional spherical shell. This is the underlying structural foundation for solar Surface Flux Transport (SFT) models.

---

## 1. The Vector Induction Equation

The time-evolution of a magnetic field $\mathbf{B}$ embedded within a plasma moving with a velocity field $\mathbf{u}$ is governed by the induction equation:
...
Where:
* $\mathbf{u}$ represents the macroscopic plasma flow velocity field (incorporating differential rotation, meridional circulation, and turbulent granulating flows).
* $\eta$ is the magnetic diffusivity coefficient ($m^2/s$).
...
To model the solar surface, we place our coordinate framework into a spherical coordinate system $(r, \theta, \phi)$, where $r$ is radius, $\theta$ is colatitude (measured from the north pole), and $\phi$ is longitude. 

We apply three physical constraints unique to Surface Flux Transport setups:
1.  **Strictly Radial Field Focus:** We isolate the evolution at a fixed solar radius ($r = R_{\odot}$). Thus, we only track the radial component $B_r(\theta, \phi, t)$.
2.  **Purely Horizontal Surface Velocity:** The plasma velocity field near the photosphere is bounded to the horizontal surface, meaning $u_r = 0$, leaving $\mathbf{u} = (0, u_\theta, u_\phi)$.
3.  **Divergence-Free Field Condition:** The magnetic field must satisfy Maxwell's constraint: $\nabla \cdot \mathbf{B} = 0$.

Expanding the vector advection term $\nabla \times (\mathbf{u} \times \mathbf{B})$ solely for its radial ($\hat{\mathbf{r}}$) component under these geometric bounds yields the classic 2D advection equation on a sphere:
...
To solve this numerically on an HPC system, we construct an equi-angular staggered or centered mesh configuration. Let our grid be spaced uniformly by $\Delta \theta$ and $\Delta \phi$, where index $i$ corresponds to colatitude $\theta_i$ and index $j$ corresponds to longitude $\phi_j$.
...
Using a Forward-Time Central-Space (FTCS) approach, the explicit temporal update for $B_r$ from timestep $n$ to $n+1$ can be expressed as:

$$
B_{r, i, j}^{n+1} = B_{r, i, j}^{n} + \Delta t \cdot \left[ \mathcal{A}_{i,j}^n + \mathcal{D}_{i,j}^n \right]
$$

Where $\mathcal{A}_{i,j}^n$ represents the combined advective terms and $\mathcal{D}_{i,j}^n$ captures the diffusive updates derived in Section 3.

### Numerical Stability Constraints (The CFL Condition)
Because this formulation is explicit, your choice of $\Delta t$ is strictly limited to prevent exponential code explosion. The timestep must satisfy both the advective Courant-Friedrichs-Lewy (CFL) constraint and the diffusion stability constraint:
...
> **Warning near the Poles:** As $\theta \to 0$ or $\theta \to \pi$ near the solar poles, the $\sin\theta$ term in the longitudinal CFL condition approaches zero. This forces $\Delta t$ to become vanishingly small, causing a severe computational bottleneck.
